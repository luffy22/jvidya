...
window.addEvent('domready',function(){
	...
	function loadNotification(){
		if(typeof Request == "function"){
			new Request.HTML({url: 'http://ads.mobilejoomla.com/msg.html?v=1.2.8', method: 'get', update: 'mjmsgarea'}).send();
		} else if(typeof Ajax == "function"){
			new Ajax('http://ads.mobilejoomla.com/msg.html?v=1.2.8', {method: 'get', evalScripts: true, update: $('mjmsgarea')}).request();
		}
	}
	...
	function checkUpdate(){
		if(typeof Request == "function"){
			new Request.HTML( {
				url: 'http://www.mobilejoomla.com/getver.php?v=' + escape('1.2.8'),
				method: 'get',
				update: 'mjlatestver',
				onSuccess : function(tree, elements, response){
					if(version_compare('1.2.8', response)>0){
						$('mjlatestverurl').setStyle('display', 'block');
					}
				}
			}).send();
		} else if(typeof Ajax == "function"){
			new Ajax( 'http://www.mobilejoomla.com/getver.php?v=' + escape('1.2.8'), {
				method: 'get',
				update: $('mjlatestver'),
